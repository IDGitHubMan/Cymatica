public class Player {
    ...
    SongList m = new SongList(cp5, "menu", 200, 368);
    ...
    Player(ControlP5 controller, Minim m, String name, JSONObject p, int num) {
        ...
        for(int i = 0; i < songList.size(); i++) {
            ...
            try {
                so = newSong(minim,s.getString("path"),s.getString("author"),s.getString("title"),s.getString("album"),s.getInt("number"),s.getInt("left"),s.getInt("right"),s.getInt("mix"));
                songs.add(so);
                fft = so.fft;
            }
            ...
            if(so!= null) {
                ...
                //Removesong button (to be added in later)
           ...
           void songAppend(File f) {
            try {
                   Song s = new Song(minim,f.getAbsolutePath(),songs.size() + 1);
                    songs.add(s);
                    Stringpath = sketchPath() + "/data/Library/" + n + "/" + f.getName();
                    createOutput(path);
                                                                           ...
                                                                           void controlEvent(ControlEvent e) {
                                                                                ...
                                                                                if(e.getName().contains("play") && e.getName() != "playPause") {
                                                                                   if(playing != null) {
                                                                                           ...
                                                                                           // (Faulty) Song Removal
                                                                                        elseif(e.getName().contains("remove")) {
                                                                                               if(playing == songs.get(Integer.parseInt(e.getName().substring(e.getName().length())))) {
                                                                                                   ...
                                                                                                   songs.remove(Integer.parseInt(e.getName().substring(e.getName().length() - 1)));
                                                                                                    ...
                                                                                                    //Seekbar
                                                                                                elseif(e.getName() == "seek" && mousePressed && e.getValue() != playing.audio.position() && seekbar.isMouseOver()) {
                                                                                                        playing.audio.pause();
                                                                                                        ...
                                                                                                        //Mute toggle
                                                                                                 elseif(e.getName() == "mute" && mousePressed && muter.isMouseOver()) {
